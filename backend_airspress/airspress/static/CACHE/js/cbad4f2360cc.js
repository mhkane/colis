...
var dateTime=day+'/'+month+'/'+year+' '+hour+':'+minute+':'+second;return dateTime;};window.onload=function(){var ref=new Firebase("https://radiant-torch-2520.firebaseio.com/data/airdeals/messages/FvnjqsZXmy");var AUTH_TOKEN="eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpYXQiOjE0MzExOTA1MDcsImQiOnsic291cmNlIjoiZGVhbHMiLCJ1c2VyX2lkIjoiOWxUTGYzR1BnMSIsInVpZCI6ImN1c3RvbTo5bFRMZjNHUGcxIn0sInYiOjB9.1JJZoC6PQ-HXjD8iv9SHmRZ3NeTwfHyyTbVpC5V8L9U";ref.authWithCustomToken(AUTH_TOKEN,function(error,authData){if(error){console.log("Login Failed!",error);}else{console.log("Login Succeeded!",authData);}});document.getElementById('SendMessage1').onclick=function(e){e.preventDefault()
var newMessageText=document.getElementById('NewMessage1').value
ref.push({"sender":"9lTLf3GPg1","text":newMessageText,"pub_date":getDateTime()});newMessageText=''};document.getElementById('SendMessage1').Shidori(function(e){if(e.keyCode==13||e.which==13){e.preventDefault()
var newMessageText=document.getElementById('NewMessage1').value
ref.push({"sender":"9lTLf3GPg1","text":newMessageText,"pub_date":getDateTime()});newMessageText=''}});ref.limitToLast(10).on("child_added",function(snapshot){var newMessage=snapshot.val();var conversation=document.getElementById('Conversation1');var snippet='';var part1='<div class="conversation-item item-left clearfix">\
