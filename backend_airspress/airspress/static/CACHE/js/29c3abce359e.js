...
var dateTime=day+'/'+month+'/'+year+' '+hour+':'+minute+':'+second;return dateTime;};window.onload=function(){var ref=new Firebase("https://radiant-torch-2520.firebaseio.com/data/airdeals/messages/FvnjqsZXmy");var AUTH_TOKEN="eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpYXQiOjE0MzExOTA4NDQsImQiOnsic291cmNlIjoiZGVhbHMiLCJ1c2VyX2lkIjoiOWxUTGYzR1BnMSIsInVpZCI6ImN1c3RvbTo5bFRMZjNHUGcxIn0sInYiOjB9.Gmt-L3iGDiXjsaCKkmUztXuMJctmz64OeOAaVDTZ-rM";ref.authWithCustomToken(AUTH_TOKEN,function(error,authData){if(error){console.log("Login Failed!",error);}else{console.log("Login Succeeded!",authData);}});document.getElementById('SendMessage1').onclick=function(e){e.preventDefault()
var newMessageText=document.getElementById('NewMessage1').value
ref.push({"sender":"9lTLf3GPg1","text":newMessageText,"pub_date":getDateTime()});newMessageText=''};document.getElementById('SendMessage1').function Shidori(e){if(e.keyCode==13||e.which==13){e.preventDefault()
var newMessageText=document.getElementById('NewMessage1').value
