...
var dateTime=day+'/'+month+'/'+year+' '+hour+':'+minute+':'+second;return dateTime;};window.onload=function(){var ref=new Firebase("https://radiant-torch-2520.firebaseio.com/data/directMessaging/conversations/9lTLf3GPg1-A2NTGJxHJr");var AUTH_TOKEN="eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpYXQiOjE0MzEyMDQ0NTQsImQiOnsic291cmNlX2lkIjoiOWxUTGYzR1BnMS1BMk5UR0p4SEpyIiwic291cmNlIjoiY2hhdCIsInVzZXJfaWQiOiI5bFRMZjNHUGcxIiwidWlkIjoiY3VzdG9tOjlsVExmM0dQZzEifSwidiI6MH0.u5EMa9ugy3q9J1klxpalGxVxmT73ekm0S8h96eyKH1g";ref.unauth();ref.authWithCustomToken(AUTH_TOKEN,function(error,authData){if(error){console.log("Login Failed!",error);}else{console.log("Login Succeeded!",authData);}});document.getElementById('SendMessage1').onclick=function(e){e.preventDefault();var newMessageText=document.getElementById('NewMessage1').value;ref.push({"sender":"9lTLf3GPg1","text":newMessageText,"pub_date":getDateTime()});newMessageText='';};function Shidori(e){if(e.keyCode==13||e.which==13){e.preventDefault();var newMessageText=document.getElementById('NewMessage1').value;if(newMessageText!==""){ref.push({"sender":"9lTLf3GPg1","text":newMessageText,"pub_date":getDateTime()});newMessageText='';}}};ref.limitToLast(10).on("child_added",function(snapshot){var newMessage=snapshot.val();var conversation=document.getElementById('Conversation1');var wrapper=document.getElementById('WrapperConversation1');var snippet='';var part1='<div class="conversation-item item-left clearfix">\
